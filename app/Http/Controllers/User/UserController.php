...
class UserController extends Controller
{
    ...
    {
        $this->report_data();
        return View('technoland.user.index');
    }
    ...
    public function orders()
    {
        ...
        return View('technoland.user.orders',['orders'=>$orders]);
    }
    ...
    public function favorites()
    {
        $this->report_data();
        $user_id=Auth::user()->id;
        $favorites=Bookmark::where(['user_id'=>$user_id])->orderBy('id','DESC')->paginate(10);
        return View('technoland.user.favorites',['favorites'=>$favorites]);
    }

    public function remove_favorites($id)
    {
        $favorites = Bookmark::find($id);
        if ($favorites) {
            $favorites->delete();
        }
        \session()->flash('success', 'محصول با موفقیت از لیست علاقه مندی ها حذف شد.');
    ...
    public function comment()
    {
        $this->report_data();
        $user_id=Auth::user()->id;
        $comments=ProductScore::with(['get_comment','get_product'])->where(['user_id'=>$user_id])->orderBy('id','DESC')->paginate(10);
        return View('technoland.user.comment',compact('comments'));
    }
    ...
    public function question()
    {
        $this->report_data();
        $user_id=Auth::user()->id;
        $question = Question::with(['get_product', 'get_user'])->where(['user_id'=>$user_id])->orderBy('id', 'DESC')->paginate(10);
        return View('technoland.user.question',compact('question'));
    }
    ...
    public function address()
    {
        $this->report_data();
        $user_id=Auth::user()->id;
        $address = Address::with('get_city')->with('get_province')->where('user_id', $user_id)->orderBy('id', 'DESC')->paginate(10);
        return View('technoland.user.address',compact('address'));
    }
    ...
    public function print_order(Request $request)
    {
        ...
        return view('technoland.user.print',['order'=>$order]);
    }
...
';
        ...
        $html.=' <tr>
                 ...
                 <td>
                    '.$order->get_address_data->get_province->ostan_name.' - '.$order->get_address_data->get_city->name.'
                 </td>
             </tr>
        ...
        $stylesheet = file_get_contents('user/css/pdf_style.css');
    ...
    public function gift_cart()
    {
        ...
        return View('technoland.user.gift_cart',['gift_cart'=>$gift_cart]);
    }
}
